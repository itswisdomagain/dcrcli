...
export default class extends Controller {
  ...
  newDestination () {
    ...
    destinationNode.setAttribute('data-index', this.destinationCount)
    addressInput.setAttribute('data-index', this.destinationCount)
    amountInput.setAttribute('data-index', this.destinationCount)
    sendMaxCheckbox.setAttribute('data-index', this.destinationCount)

    // make clicking on the label toggle the checkbox by setting unique id
    destinationTemplate.querySelector('.form-check-label').setAttribute('for', `send-max-amount-${this.destinationCount}`)
    sendMaxCheckbox.setAttribute('id', `send-max-amount-${this.destinationCount}`)
  ...
  removeDestination () {
    if (this.destinationCount > 1) {
      const lastDestinationNode = this.destinationsTarget.querySelector('div.destination:last-child')
      const lastAmount = parseFloat(lastDestinationNode.querySelector('input[name="destination-amount"]').value)

      const lastAddressIndex = lastDestinationNode.getAttribute('data-index')
      if (this.maxSendDestinationIndex >= lastAddressIndex) {
        // previously set max send destination is removed, enable other checkboxes
        this.maxSendDestinationIndex = -1
        this.maxSendAmountCheckTargets.forEach(checkbox => {
          checkbox.removeAttribute('readonly')
          checkbox.parentElement.classList.remove('disabled')
      ...
      this.destinationsTarget.removeChild(lastDestinationNode)
      this.destinationCount--

      if (!isNaN(lastAmount) && lastAmount > 0) {
        // update max send amount field if the destination field that was removed had a positive send amount
        this.updateMaxAmountFieldIfSet()
      }
    }
